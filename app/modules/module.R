    ...
    observeEvent(input$lancer, {
        ...
        time <- system.time({
            ...
            for(i in 1:length(input$s_input1)){
                ...
                d <- as.data.frame(d)
                data <<- d
                name <<- paste0(sep[1,1],"-",sep[2,1],".xlsx",sep="")
                write_xlsx(d, paste0("../modifications/",sep[1,1],"-",sep[2,1],".xlsx",sep=""))
                reportDone(TRUE)
            }
        })
        ...
        shinyalert("Modifications terminées",paste0("Nombre de lignes traitées : ",n,"\nTemps : ",time,"\nDestination du fichier : modifications/",sep[1,1],"-",sep[2,1],".xlsx\n \nVous pouvez retélécharger le fichier traité si vous le souhaitez"), type = "success", immediate = TRUE)
    })
